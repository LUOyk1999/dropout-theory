...
def dirichlet_energy_sparse(x, adj):
    with torch.no_grad():
        # Convert the adjacency matrix to SparseTensor if not already
        ...
        deg = adj.sum(dim=1).to(torch.float32)  # summing over column to get the degree of each node
        
        # Normalize features
        x = x / torch.sqrt(deg + 1.0).unsqueeze(-1)
        
        # Compute the difference of features between connected nodes
        row, col, _ = adj.coo()  # Get the COO format edges
        diff = x[row] - x[col]
        
        # Calculate the norm squared
        norm_sq = (diff.norm(dim=1) ** 2).mean()  # Taking the mean over all edges
...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        # print(dirichlet_energy_sparse(x,edge_index))
        if self.jk:
            x = x_local
        else:
            pass
